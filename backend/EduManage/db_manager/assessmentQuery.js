const QUERY = 
{
    ...
    GET_SCHEDULED_ASSESSMENTS : 'Select OpenDate, CloseDate, Duration, Participants, Marks, Questions, CoverImage, CASE WHEN OpenDate <= NOW() AND CloseDate >= NOW() THEN "In Progress" WHEN OpenDate > NOW() THEN "Not Started" ELSE "Requires Grading" END AS Status from Assessment where TeacherID = ? AND Status != "Published"',
