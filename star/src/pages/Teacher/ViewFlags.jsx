          ...
          <div className={`p-2 md:p-4 flex gap-4 overflow-hidden ${loading ? 'h-full flex-row justify-center items-center' : 'flex-col'}`}>
            {
              ...
              <div className='w-full bg-LightBlue flex p-2 items-center justify-between shadow-md'>
                ...
                <table className='w-full text-sm md:text-base border-separate border-spacing-y-2'>
                    ...
                    <tbody>
                        {flaggings.map((row, rowIndex) => (
                        <tr key={rowIndex} className={`text-center shadow-md bg-LightBlue`}>
                            <td className={`px-1 py-2 text-xs md:text-sm border-black border-y-[1px] ${row.type == 'tab switch' ? 'text-DarkBlue' : row.type == 'More than one person on screen' ? 'text-red-500' : 'text-indigo-400'}`}>
                                <p className={`p-2 border-[1px] w-fit rounded-full ${row.type == 'tab switch' ? 'border-DarkBlue' : row.type == 'More than one person on screen' ? 'border-red-500' : 'border-indigo-400'}`}>{row.type}</p>
                            </td>
