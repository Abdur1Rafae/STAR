import React, { useState, useEffect } from 'react'
import MenuBar from '../../components/MenuBar'
import SideBar from '../../components/Teacher/SideBar'
import Subheader from '../../components/Teacher/Subheader'
import Loader from '../../components/Loader'
import { BiChevronLeft } from 'react-icons/bi'

const ViewFlags = () => {
  const [loading, setLoading] = useState(false)

  useEffect(()=>{

  }, [])

  const data = [{category: 'Tab Switch', timeStamp: '2024-11-24 16:45', img: 'data:image/jpeg;base64,'}, {category: 'Two Faces Detected', timeStamp: '2024-11-24 16:45', img: ''}
,{category: 'Object Detected', timeStamp: '2024-11-24 16:45', img: ''},{category: 'Off-screen during quiz', timeStamp: '2024-11-24 16:45', img: ''}]
  
  
  return (
    <div className=' w-full h-full font-body'>
      <MenuBar name={"Jawwad Ahmed Farid"} role={"Teacher"}/>
      <div className='w-auto md:h-full flex md:flex-row flex-col-reverse'>
         <SideBar active={"Grading"}/>
         <div className='w-full h-full'>
          <Subheader name={"Grading"}/>
          <div className={`p-2 md:p-4 flex gap-4 overflow-hidden ${loading ? 'h-full flex-row justify-center items-center' : 'flex-col'}`}>
            {
              loading ?
              <Loader/>
              :
              <>
              <div className='w-full bg-LightBlue flex p-2 items-center justify-between shadow-md'>
                  <div className='w-full flex justify-between items-center self-start'>
                    <div className='flex items-center self-start'>
                      <button onClick={()=>{window.location.assign('/teacher/home')}}><BiChevronLeft className='text-3xl'/></button>
                      <h4 className='font-semibold'>Abdur Rafae</h4>
                    </div>
                  </div>
                  </div>
                <table className='w-full text-sm md:text-base border-separate border-spacing-y-2'>
                    <thead className=''>
                        <tr className='text-center text-gray-600'>
                            <th className='px-1 py-3 tracking-wide border-b-[1px] border-[#937D7D]'>
                                Category
                            </th>
                            <th className='px-1 py-3 tracking-wide border-b-[1px] border-[#937D7D]'>
                                TimeStamp
                            </th>
                            <th className='px-1 py-3 tracking-wide border-b-[1px] border-[#937D7D]'>
                                Proof
                            </th>
                        </tr>
                    </thead>

                    <tbody>
                        {data.map((row, rowIndex) => (
                        <tr key={rowIndex} className={`text-center shadow-md bg-LightBlue`}>
                            <td className={`px-1 py-2 text-xs md:text-sm border-black border-y-[1px] ${row.category == 'Tab Switch' ? 'text-DarkBlue' : row.category == 'Two Faces Detected' ? 'text-red-500' : row.category == 'Object Detected'? 'text-yellow-600' : 'text-indigo-400'}`}>
                                <p className={`p-2 border-[1px] w-fit rounded-full ${row.category == 'Tab Switch' ? 'border-DarkBlue' : row.category == 'Two Faces Detected' ? 'border-red-500' : row.category == 'Object Detected'? 'border-yellow-600' : 'border-indigo-400'}`}>{row.category}</p>
                            </td>
                            <td className='px-1 py-2 text-xs md:text-sm border-black border-y-[1px]'>
                                {row.timeStamp}
                            </td>
                            <td className='px-1 py-2 text-xs md:text-sm border-black border-y-[1px] flex items-center justify-center'>
                                <img src={row.img} className='w-48 h-48'/>
                            </td>
                        </tr>
                        ))}
                    </tbody>  
                </table>
              </>
            }
          </div>
        </div>
      </div>
   </div>
);
}

export default ViewFlags