                ...
                data.map((assessment, index) => {
                    return (
                        <div className='w-full mb-2'>
                            <PastQuizCard previousScore={index > 0 ? data[index - 1].totalMarks : '-'} prevTotalScore={index > 0 ? (data[index-1].responseId ? (data[index - 1].totalScore ? data[index-1].totalScore : '-') : 0) : '-'} showReport={assessment.totalScore} id={assessment.responseId} AchievedScore={assessment.responseId ? assessment.totalScore ? assessment.totalScore : 'pending' : 0} totalScore={assessment.totalMarks} Date={assessment.submitted ? DDMMM_HHMM(assessment.submitted) : '-'} Name={assessment.title}/>
                        </div>
                    )
                })
            }
